...
<div class="row clearfix">
    <div class="col-xs-12 col-lg-12" style="border-bottom:2px ##428bca solid;">
        <h1 class="blue" >#getLabel("Monthly Report", #SESSION.languageId#)#
...
<script type="text/javascript">
	...
	function changeMonth(action){
		...
		window.history.pushState('Filter', 'Filter', "/index.cfm/report/monthly?date="+cYear+"-"+cMonth );
        getData();
	}
	...
	function getData(){
		$.ajax({
			type: 'get',
			url: '/index.cfm/report/loadMonthly?date='+cYear+"-"+cMonth,
			dataType : 'html',
			beforeSend: function() {
