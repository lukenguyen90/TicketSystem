 ...
 *
 * //change by Saso
 * 386: Added class to the current day td (td-current-day) to be used for styling purposes
 *
 * Nikola's Changes
 * Task: TDWEBOLD-3771:
 * Date 18 March 2014
 * Line: 321:326
 * changes: Week start from Monday
        ...
        DateTimePicker = function (element, options) {
            var defaults = {
                ...
                init = function () {
                    ...
                    picker.date = pMoment();
                    if(picker.options.setCurrentDay!='') {
                        //if this is not set use client time
                        picker.date = pMoment(picker.options.setCurrentDay);
                    }
                    else {
                        //if setCurrentDay is not set, we set the default date to clients date
                        picker.date = pMoment();
                    }
                    picker.unset = false;
                    ...
                    picker.widget = $(getTemplate(picker.options.pickDate, picker.options.pickTime, picker.options.collapse, picker.options.viewMode)).appendTo(picker.options.parentElement);
                ...
                fillDate = function () {
                    ...
                    while (prevMonth.isBefore(nextMonth)) {
                        ...
                        if(picker.options.setCurrentDay!='') {
                            todaysDate = pMoment(picker.options.setCurrentDay).format('MMMM DD, YYYY')
                        }
                        else {
                            //if setCurrentDay is not set, we set the default date to clients date
                            todaysDate = pMoment().format('MMMM DD, YYYY')
                        }


                        if(todaysDate == pMoment(prevMonth).format('MMMM DD, YYYY') && !prevMonth.isSame(pMoment({ y: picker.date.year(), M: picker.date.month(), d: picker.date.date() }))){
                            //console.log(pMoment().format('MMMM DD, YYYY'));
                            row.append('<td class="day' + clsName + ' td-current-day"><div class="current-date">' + prevMonth.date() + '</div></td>');
                        }else {
                ...
                getTemplate = function (pickDate, pickTime, collapse, pickerViewMode) {
                    if(pickerViewMode==='months'){
                        monthsClass = 'month-selection';
                    }
                    else {
                        monthsClass = '';
                    }
                    if (pickDate && pickTime) {
                        ...
                        return (
                            '<div class="bootstrap-datetimepicker-widget dropdown-menu '+monthsClass+'">' +
                                '<div class="datepicker">' + dpGlobal.template + '</div>' +
                                '<div class="select-today"><a href="javascript:;" class="select-today-button">Today</a><a href="javascript:;" class="select-week-button">This week</a></div>' +
                                '</div>'
                            );
                    }
                },
