...
!function($) {
    ...
    Multiselect.prototype = {
        ...
        defaults: {
            ...
            buttonText: function(options, select) {
                if (options.length === 0) {
                    ...
                    if (/multiselect-all/i.test(select.val())){
        ...
        updateSelectAll: function() {
            ...
            //if one is deselected from the visible options deselect the select-all checkbox, this fixes selecting all when in filtering mode
            var totalVisible = $('option', this.$select).length;//this.$ul.find('li:visible').length-1;
            //select total selected wich are visible at the moment and with check all not chacked
            var totalSelected = (this.$select.val() != null) ? this.$select.val().length : 0 ;//this.$ul.find('li:visible input[value!=multiselect-all]:checked').length;
            var hasAll =  (/multiselect-all/i.test(this.$select.val()));
            
            if(totalVisible !== totalSelected) {
                if(hasAll) {
                    this.$select.val(this.$select.val().splice(1, this.$select.val().length));
                    //console.log(this.$select.val());
                }
                this.deselect(this.options.selectAllValue);
            }
            else if(totalVisible === totalSelected) {
                this.select(this.options.selectAllValue);
            }
        },
         ...
         * @returns {jQUery}
         */
        setUsersSelectedToDB: function(page) {
            this.updateSelectAll();
            setUsersSelected(this.$select.val(), page);
        },
